(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".hero__slider")) {
            let swiperSpeed = 600;
            const firstLinePosition = document.querySelector(".hero__line--1").attributes.y1.nodeValue;
            ...
            function setActiveThumb(activeIndex) {
                ...
                allThumbs.forEach(((thumb, index) => {
                    ...
                    if (index === activeIndex) {
                        ...
                        if (line && (line.attributes.y1.nodeValue == 0 || line.attributes.y2.nodeValue - line.attributes.y1.nodeValue > 150)) line.attributes.y1.nodeValue = parseInt(line.attributes.y1.nodeValue) + 100;
                    } else {
                        thumb.classList.remove("active");
                        stat.style.opacity = 0;
                        if (line && index == 1) line.attributes.y1.nodeValue = firstLinePosition; else if (line && index == 4) line.attributes.y1.nodeValue = secondLinePosition;
                    }
                }));
            }
    ...
    Array.from(document.querySelectorAll(".js-accordion")).forEach((el => {
        ...
        accordion.toggleItem(2, true, {
            noTransition: true
        });
    }));
    const menuBody = document.querySelector(".header__menu--mobile");
    const menuBtn = document.querySelector(".header__burger");
    const body = document.body;
    if (menuBody && menuBtn) menuBtn.addEventListener("click", (function() {
        menuBody.classList.toggle("active");
        menuBtn.classList.toggle("burger--active");
        body.classList.toggle("lock");
    }));
    const countOfAboutItems = () => {
        ...
        const resizeObserver = new ResizeObserver((entries => {
            for (const entry of entries) {
                ...
                items.forEach((item => {
                    item.classList.remove("next-wrapped");
                }));
                if (items.length > itemsPerRow) {
                    const lastIndex = itemsPerRow - 1;
                    items[lastIndex].classList.add("next-wrapped");
                    if (itemsPerRow === 1) items.forEach(((item, index) => {
                        if (index !== items.length - 1) item.classList.add("next-wrapped");
                    }));
                }
            }
        }));
